...
const statlabLocationFull = 'Statlab, Inc. Fancom Medical Plaza, #30 Quezon Street';
...
function normalizeNoteLocation(location) {
  ...
  if (trimmedLocation === 'Statlab, Inc. Fancom Medical Plaza,' || trimmedLocation === 'Statlab, Inc. Fancom Medical Plaza') {
    return statlabLocationFull;
  }

  if (trimmedLocation.startsWith('Statlab, Inc. Fancom Medical Plaza,') && !trimmedLocation.includes('#30 Quezon Street')) {
    return statlabLocationFull;
  }
...
function renderNotes() {
  const visibleNotes = notes
    ...
    .filter(({ note }) => `${note.title || ''} ${note.subject || ''} ${note.location || ''} ${note.link || ''}`.toLowerCase().includes(searchQuery));
  ...
  const activeField = activeElement && activeElement.classList.contains('note-title-input')
    ? 'title'
    : activeElement && activeElement.classList.contains('note-subject-input')
      ? 'subject'
      : activeElement && activeElement.classList.contains('note-location-input')
        ? 'location'
  ...
  notesTableBody.innerHTML = visibleNotes.length
    ? visibleNotes.map(({ note, index }) => `
      <tr data-note-id="${note.id}">
        ...
        <td><input class="note-subject-input" type="text" value="${escapeAttribute(note.subject)}" aria-label="Note subject"></td>
        <td><input class="note-location-input" type="text" value="${escapeAttribute(note.location)}" aria-label="Note location"></td>
    ...
    : `<tr><td colspan="8">${notes.length ? 'No matching notes found.' : 'No notes yet. Click Add Note to create one.'}</td></tr>`;
...
async function saveNote(row) {
  ...
  const subject = row.querySelector('.note-subject-input').value.trim();
  const location = row.querySelector('.note-location-input').value.trim();
  const link = row.querySelector('.note-link-input').value.trim();
  ...
  const normalizedLocation = normalizeNoteLocation(location);
...
firebaseAuthReady
  .then(() => notesCollection.orderBy('createdAt', 'asc').onSnapshot(snapshot => {
    notes = snapshot.docs.map(document => normalizeNote({ id: document.id, ...document.data() }));
    localStorage.setItem(notesCacheStorageKey, JSON.stringify(notes));
    renderNotes();
